...
- Application layer protocols include HTTP,DNS, Websockets, WebRTC as well as SMTP.
...
- at this layer we have TCP, QUIC and UDP which provides end-to-end communication services.
...
- `InfiniBand` protocol -  used extensively for massive ML training workloads
...
1. Generic top-level domains: These are domains that are not country specific, some of the best-known generic TLDs include .com, .org, .net, .edu, and .gov.
